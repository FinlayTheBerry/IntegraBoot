...
root_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "systemd")
os.chdir(root_dir)
print(os.getcwd())
...
for path in srcs:
    ...
    protected_paths.add(full_path)
    parent = os.path.dirname(full_path)
    while parent.startswith(root_dir):
...
for current_dir, subdirs, files in os.walk(root_dir, topdown=False):
    ...
    for subdir in subdirs:
        ...
        if subdir_path not in protected_paths:
