...
static EFI_STATUS find_sections(
    ...
    EFI_STATUS err = EFI_ERROR_INVALID;

    assert(loaded_image);
    assert(sections);

    const PeSectionHeader *section_table = NULL;
    size_t n_section_table = 0;
...
static EFI_STATUS run(EFI_HANDLE image) {
    ...
    /* Find the sections we want to operate on and return an error if any sections are missing. */
    ...
    /* Random seed is stored in ESP to reduce wear on NVRAM which has limited write cycles */
    refresh_random_seed(loaded_image);

    /* TBH not 100% sure why we are mangling the stub cmdline but it seems to remove trailing null byte spam. */
    cmdline = mangle_stub_cmdline(pe_section_to_str16(loaded_image, sections + UNIFIED_SECTION_CMDLINE));
